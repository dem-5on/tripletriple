...
logger = logging.getLogger("openclaw.agents.skills")
...
class SkillEntry(BaseModel):
    ...
    # OpenClaw metadata bag
    metadata: Dict[str, Any] = Field(default_factory=dict)


class SkillConfigEntry(BaseModel):
    """Per-skill config override from openclaw.json."""
...
def _parse_skill_md(path: Path, location: str) -> Optional[SkillEntry]:
    ...
    oc_meta = fm.get("metadata", {}).get("openclaw", {})
...
class SkillLoader:
    ...
    def load_all(self) -> List[SkillEntry]:
        ...
        # Tier 3: Managed skills (~/.openclaw/skills/)
    ...
    def format_for_prompt(skills: List[SkillEntry]) -> str:
        ...
        matching OpenClaw's `formatSkillsForPrompt` format.
